...
For solver work, read `simulation/README.md` and the relevant benchmark input. The simulation project has a separate lock. Run `uv run --project engineering/simulation --locked python engineering/simulation/run.py benchmark` for actual solver verification, and its unittest suite for input/provenance failure cases. `run.py check <report>` only checks saved evidence freshness. Do not report a solver/platform as tested based on release availability or a successful installation alone. Keep native solver warnings and distinguish ideal thermal loads from electricity consumption.
